...
static void error_handler(MEM_Controller controller,char *filename,int line,char *msg)
{
	...
	if(controller->fail_mode == MEM+MEM_FAIL_AND_EXIT)
	{
		exit(1);
	}
}
...
void check_mark(Header *header)
{
	...
	tail = ((unsigned char*)header) +header->size +sizeof(Header);
	...
	void *ptr, size_t size)
{
	...
	if(new_ptr == NULL)
	{
		if(ptr == NULL)
		{
			error_handler(controllerm,filename,line,"realloc(malloc)");
		}
		else
		{
...
void MEM_check_all_blocks_func(MEM_Controller counter,
